...
export default {
  ...
  async created() {
    ...
    this.balances = this.allAppareils.filter(appareil => appareil.type.nom === 'Balance');
  ...
  methods: {
    ...
    ...mapActions('appareils', ['loadAppareils', 'loadInstructionsAppareils', 'getCalibration']),
    ...
    handleInput() {
        console.log('Input event triggered. Selected appareil ID:', this.appareil.id);
        if (this.appareil && this.appareil.id) {
            this.getCalibration(this.appareil.id);
        } else {
